...
For a finite simple graph G with n vertices and m edges, let λ₁≥⋯≥λₙ be the eigenvalues of its adjacency matrix. Its energy is E(G)=∑ᵢ|λᵢ|.
...
which is impossible because 20<24. Also det(A)=1, so the counterexample satisfies the nonsingularity hypothesis.
...
The separate Conjecture 1 in the 2025 paper is unaffected; a proof is given in [arXiv:2608.22139](https://arxiv.org/abs/2608.22139).

The Lean certificate [P4EnergyCounterexample.lean](https://github.com/advpropsys/lean-stuff/blob/main/graph-energy-p4/Research/P4EnergyCounterexample.lean) verifies the graph, connectivity, edge count, adjacency matrix, characteristic polynomial, complete root multiset, root extrema, determinant, energy, and failure of the graph-level bound. Its only axioms are Lean's standard propext, Classical.choice, and Quot.sound.
